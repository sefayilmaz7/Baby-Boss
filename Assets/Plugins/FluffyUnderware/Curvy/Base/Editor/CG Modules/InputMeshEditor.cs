    ...
    public class InputMeshEditor : CGModuleEditor<InputMesh>
    {
        ...
        string getFormattedMeshInfo(Mesh msh)
        {
            if (msh)
            {
                ...
                string close = "</color>";
                string norm = (msh.normals.Length > 0) ? has : hasnt;
                string tan = (msh.tangents.Length > 0) ? has : hasnt;
                string uv = (msh.uv.Length > 0) ? has : hasnt;
                string uv2 = (msh.uv2.Length > 0) ? has : hasnt;
