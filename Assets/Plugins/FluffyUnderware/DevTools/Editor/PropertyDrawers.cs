...
public class VectorExPropertyDrawer : DTPropertyDrawer<VectorExAttribute>
{
    ...
    public static void DoGUI(Rect position, SerializedProperty property, DTPropertyAttribute attribute,
        GUIContent label, AttributeOptionsFlags flags, VectorExPropertyDrawer drawer = null)
    {
        ...
        else
        {
            ...
            if (flags.HasFlag(AttributeOptionsFlags.Compact) || EditorGUIUtility.wideMode)
            {
                ...
                if (property.propertyType == SerializedPropertyType.Vector2)
                {
                    ...
                    if (EditorGUI.EndChangeCheck())
                        property.vector2Value =
                            DT._UseSnapValuePrecision ? DTMath.SnapPrecision(v, attribute.Precision) : v;
                }
                ...
                {
                    EditorGUI.BeginChangeCheck();
                    Vector3 v = DTGUI.CompactVector3Field(r,
                        DT._UseSnapValuePrecision
                            ? DTMath.SnapPrecision(property.quaternionValue.eulerAngles, attribute.Precision)
            ...
            if (GUI.Button(position, "..", EditorStyles.miniButton))
            {
                ...
                switch (Attribute.Mode)
                {
                    ...
                    case PathSelectorAttribute.DialogMode.CreateFile:
                        v = EditorUtility.SaveFilePanel(Attribute.Title, Attribute.Directory, Attribute.DefaultName,
                            Attribute.Extension);
                        break;
                }
...
public class FloatRegionPropertyDrawer : DTPropertyDrawer<FloatRegionAttribute>
{
    ...
    void doMinmax(Rect r, GUIContent label, bool showOptional)
    {
        ...
        if (showOptional)
        {
            ppSimpleValue.boolValue = !EditorGUI.Toggle(new Rect(r.xMax - 16, r.y, 16, r.height),
                new GUIContent("", mOptions.OptionalTooltip), !ppSimpleValue.boolValue);
        }
    ...
    void doCompact(Rect r, GUIContent label, GUIContent labelTo, bool showOptional, bool fullLine = false)
    {
        ...
        if (showOptional)
        {
            ppSimpleValue.boolValue = !EditorGUI.Toggle(new Rect(r.x, r.y, 16, r.height),
                new GUIContent("", mOptions.OptionalTooltip), !ppSimpleValue.boolValue);
            r.x += 16;
        }
        // Show "To"?
    ...
    void doRegular(Rect r, GUIContent label)
    {
        ...
        if (Attribute.RegionIsOptional)
            ppSimpleValue.boolValue = !EditorGUI.Toggle(new Rect(rF.x + rF.width + 1, rF.y, 16, rF.height),
                new GUIContent("", mOptions.OptionalTooltip), !ppSimpleValue.boolValue);
    ...
    void Init(SerializedProperty property)
    {
        ...
        {
            DTLog.LogError("[DevTools] FloatRegionPropertyDrawer: Unable to find property '" +
                           Attribute.RegionOptionsPropertyName + "'! (" + e.ToString() + ")");
        }
...
public class IntRegionPropertyDrawer : DTPropertyDrawer<IntRegionAttribute>
{
    ...
    void doRegular(Rect r, GUIContent label)
    {
        ...
        // Perhaps To is optional
        if (Attribute.RegionIsOptional)
            ppSimpleValue.boolValue = !EditorGUI.Toggle(new Rect(rF.x + rF.width, rF.y, 16, rF.height),
                new GUIContent("", mOptions.OptionalTooltip), !ppSimpleValue.boolValue);
    ...
    void Init(SerializedProperty property)
    {
        ...
        {
            DTLog.LogError("[DevTools] IntRegionPropertyDrawer: Unable to find property '" +
                           Attribute.RegionOptionsPropertyName + "'! (" + e.ToString() + ")");
        }
