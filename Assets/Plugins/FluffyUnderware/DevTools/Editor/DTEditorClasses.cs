...
public class DTInspectorNode : IComparable
{
    ...
    public string Path
    {
        get { return (Parent) ? Parent.Path + "/" + Name : Name; }
    }
    ...
    {
        get { return Items.Find(x => x.Name.Equals(name, StringComparison.CurrentCultureIgnoreCase)); }
    }
...
public class DTGroupNode : DTInspectorNode
{
    ...
    {
        get { return mState.faded; }
    }

    public override bool ContentVisible
    {
        get { return base.ContentVisible && ExpandedFaded != 0; }
        set { base.ContentVisible = value; }
    }

    public override bool NeedRepaint
    {
        get { return mState.isAnimating; }
    }
    ...
    {
        get { return Count > 0 ? Mathf.Clamp(mSelectedItem, 0, Count - 1) : -1; }
    ...
    public DTGroupNode(string name, SerializedProperty forProperty = null,
        RenderAsEnum renderAs = RenderAsEnum.Section) : base(name)
    {
        RenderAs = renderAs;
        MaxItemsPerRow = -1;
        if (forProperty != null)
    ...
    public override DTInspectorNode Add(DTInspectorNode node)
    {
        ...
        if (grp != null && _serializedObject != null && _serializedObject.targetObject != null)
            grp.mState.value =
                DTPersistentState.GetBool(_serializedObject.targetObject.GetInstanceID().ToString() + grp.Path,
                    grp.mState.value);
...
public interface IDTInspectorNodeRenderer
{
    ...
    void RenderAction(DTInspectorNode node, ActionAttribute action, System.Object editorObject,
...
public class DTInspectorNodeDefaultRenderer : IDTInspectorNodeRenderer
{
    ...
    public static GUIStyle TabbarButton
    {
        get
        {
            if (mTabbarButton == null)
            {
                ...
                mTabbarButton.padding = new RectOffset(skinButtonPadding.left, skinButtonPadding.right,
                    skinButtonPadding.top, skinButtonPadding.bottom);
                mTabbarButton.margin = new RectOffset(skinButtonMargin.left, skinButtonMargin.right,
                    skinButtonMargin.top, skinButtonMargin.bottom);
                mTabbarButton.border = new RectOffset(skinButtonBorder.left, skinButtonBorder.right,
                    skinButtonBorder.top, skinButtonBorder.bottom);
                mTabbarButton.overflow = new RectOffset(skinButtonOverflow.left, skinButtonOverflow.right,
                    skinButtonOverflow.top, skinButtonOverflow.bottom);
            }
    ...
    public virtual void RenderAction(DTInspectorNode node, ActionAttribute action, System.Object editorObject,
        System.Object targetObject)
    {
...
#pragma warning restore 162
    public static int RenderHeader(Rect controlRect, int xOffset, string helpUrl, GUIContent toggleGuiContent,
        ref bool toggleState)
    {
        ...
        Rect boxRectangle = new Rect(controlRect.x, controlRect.y - 2, controlRect.width + xOffset + 1,
    ...
    public virtual void RenderTabBarHeader(DTGroupNode node, int maxItemsPerRow)
    {
        ...
        int s = GUILayout.SelectionGrid(node.SelectedIndex, node.GetItemsGUIContent(),
            Mathf.Min(node.Count, maxItemsPerRow), TabbarButton);
