...
public abstract class DTToolbarItem : IComparable
{
    ...
    {
        get { return mItemRect.Contains(DTGUI.MousePosition); }
    }
    ...
    protected DTToolbarItem()
    {
        ...
        if (attribs.Length > 0)
        {
            ...
            {
                mIconPackedString = (!EditorGUIUtility.isProSkin && !string.IsNullOrEmpty(a.IconLightSkin))
                    ? a.IconLightSkin
                    : a.Icon;
            }
    ...
    /// </summary>
    protected void Advance(ref Rect r)
    {
        Advance(ref r, r.height);
    }
    ...
    /// </summary>
    protected void AdvanceBelow(ref Rect r)
    {
        AdvanceBelow(ref r, r.height);
    }
...
public class ToolbarItemAttribute : Attribute
{
    ...
    public ToolbarItemAttribute(int order, string project, string label, string tooltip = "", string icon = "",
        string iconLight = "") : this(project, label, tooltip, icon, iconLight, order)
    {
    }

    public ToolbarItemAttribute(string project, string label, string tooltip = "", string icon = "",
        string iconLight = "", int order = 0)
    {
        Project = project;
        Label = label;
        Tooltip = tooltip;
