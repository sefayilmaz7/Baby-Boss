        ...
        foreach (DTProject project in projects)
        {
            ...
            for (int i = 0; i < items.Count; i++)
            {
                if (items[i].Visible)
                {
                    ...
                    _ItemRect[side] =
                        advanceItemRect(lastItem, items[i],
                            itemSize); // advance by using the last itemRect and the new(current) size
        ...
        for (int side = 0; side < 4; side++)
        {
            ...
            switch (side)
            {
                ...
                default: // Bottom
                    ...
                    r.x = 5;
                    r.y = SceneView.currentDrawingSceneView.position.height - _MaxItemDimension[side].y - 30;
                    if (lft > 0)
                        r.x += 10 + lft;
                    break;
            }
    ...
    static Rect advanceItemRect(DTToolbarItem lastItem, DTToolbarItem newItem, Vector2 newItemSize)
    {
        ...
        switch (newItem.Project.ToolbarOrientation)
        {
            ...
            case DTToolbarOrientation.Top:
                ...
                if (itemRect.x + newItemSize.x > SceneView.currentDrawingSceneView.position.width -
                    _MaxItemDimension[(int) DTToolbarOrientation.Left].x -
                    _MaxItemDimension[(int) DTToolbarOrientation.Right].x)
                {
