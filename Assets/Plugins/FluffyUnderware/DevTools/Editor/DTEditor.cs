...
public class DTEditor<T> : Editor where T : UnityEngine.Object
{
    ...
    {
        get { return (target != null) ? target as T : null; }
    }
                    ...
                    switch (ev.type)
                    {
                        case EventType.DragUpdated:
                            if (r.Contains(ev.mousePosition))
                            {
                                ...
                                bool allowed = DragAndDrop.objectReferences.Length > 0 &&
                                               DTEditorUtility.DragDropTypeMatch(fieldType);
                                DragAndDrop.visualMode =
                                    allowed ? DragAndDropVisualMode.Copy : DragAndDropVisualMode.Rejected;
                            }
                        ...
                        case EventType.DragPerform:
                            ...
                            {
                                Object[] objs =
                                    DTEditorUtility.DragDropGetObjectsOfType(node.serializedProperty.GetFieldType());
            ...
            if (iterator.name != "m_Script" && iterator.name != "InspectorFoldout")
            {
                ...
                foreach (IDTGroupParsingAttribute ga in groupParseAttribs)
                {
                    if (ga is TabAttribute)
                    {
                        ...
                        if (!string.IsNullOrEmpty(tabA.TabBarName) && !string.IsNullOrEmpty(tabA.TabName))
                        {
                            if (!parentNode[tabA.TabBarName])
                                parentNode = (DTGroupNode) parentNode.Add(new DTGroupNode(tabA.TabBarName, null,
                                    DTInspectorNode.RenderAsEnum.TabBar));
                            else
                                parentNode = (DTGroupNode) parentNode[tabA.TabBarName];
                            if (!parentNode[tabA.TabName])
                                parentNode = (DTGroupNode) parentNode.Add(new DTGroupNode(tabA.TabName, iterator,
                                    DTInspectorNode.RenderAsEnum.Tab));
                            else
                            ...
                            DTLog.LogWarningFormat(
                                "[DevTools] Skipping [Tab] on '{0}' because Path is missing TabBar or Tab!",
                    ...
                    else if (ga is AsGroupAttribute)
                    {
                        ...
                        parentNode = createGroup((asGroupA.PathIsAbsolute) ? baseNode : parentNode,
                            (asGroupA.Path == null) ? fieldNode.Name : asGroupA.Path + "/" + fieldNode.Name, iterator);
                        baseNode = parentNode;
                    }
                ...
                foreach (IDTFieldParsingAttribute pa in parsingAttributes)
                {
                    ...
                    else if (pa is ArrayExAttribute)
                    {
                        ...
                        fieldNode.ArrayEx = new ReorderableList(serializedObject, iterator, arrayA.Draggable,
                            arrayA.ShowHeader, arrayA.ShowAdd, arrayA.ShowDelete);
                ...
                if (group.Visible)
                {
                    ...
                    switch (item.RenderAs)
                    {
                        ...
                        case DTInspectorNode.RenderAsEnum.TabBar:
                            NodeRenderer.RenderTabBarHeader(group,
                                (group.MaxItemsPerRow == -1) ? group.Items.Count : group.MaxItemsPerRow);
